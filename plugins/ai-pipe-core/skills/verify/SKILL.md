...
4. **verifier 디스패치** (`subagent_type: verifier`): slug + impl 출력 파일 경로들 + qa/test 결과 + reviewer 출력을 전달. verifier 는 Concordance Gate(`scripts/validate/validate-impl-concordance.sh`)와 세 신호를 종합해 `{decision: ship|no-ship, blockers, concordance}` 를 반환.
