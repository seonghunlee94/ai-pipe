...
1. **사전 확인**: spec/plan/impl 출력이 존재하는지 확인 (없으면 위 안내 후 중단). plan 의 `## Tasks` 표에서 `task_id`, `covers`, agent 레이어를 읽는다 (`task_id` 목록은 step 4 의 impl-출력 필터에 쓰인다).
...
5. **이벤트 기록**: `.artifacts/runs/{slug}-events.jsonl` 에 `phase_start`/`phase_done` (phase: "verify") 및 각 에이전트의 `task_start`/`task_done` 을 append (`mkdir -p .artifacts/runs` 선행). **키 이름은 정확히 다음 형태** — 종류 키는 `type`, task 라인은 `task_id`/`status` (dogfood: `agent`/`verdict` 식 자유 키는 `adp-watch` 가 `✔ ? ?` 로 렌더):
   ```json
   {"ts":"...","type":"phase_start","phase":"verify"}
   {"ts":"...","type":"task_start","task_id":"qa","agent":"qa"}
