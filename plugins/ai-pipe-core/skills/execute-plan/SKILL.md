...
1. **Plan 파싱**: `.artifacts/plans/{slug}-plan.md` 에서 plan 헤더의 `feature_branch` 와 `## Tasks` 표를 읽는다 — 표 구조는 `${CLAUDE_PLUGIN_ROOT}/shared/formats/plan-format.md`(SSOT)를 따른다. 각 task 는 `task_id`, `agent`(backend-eng/frontend-eng/infra-eng), `task_branch`, `depends_on`, `covers` 를 가진다.
   - **stub 에이전트 경고**: 배정된 `agent` 의 정의 파일(`${CLAUDE_PLUGIN_ROOT}/agents/{agent}.md`)이 stub(빈 `tools: []` 또는 TODO 본문)이면 그 task 로 fan-out 하지 말고 사용자에게 "해당 에이전트 미구현"을 보고하고 멈춘다 (어떤 에이전트가 stub 인지는 정의 파일을 직접 검사해 판단 — 하드코딩 금지). 빈 에이전트에 보내면 무의미한 실패→retry 루프만 생긴다.
...
7. **이벤트·출력 기록**: 각 단계를 `.artifacts/runs/{slug}-events.jsonl` 에 append. **키 이름은 정확히 다음 형태** — 이벤트 종류 키는 `type` 이다 (`event` 가 아님 — dogfood 에서 실제 드리프트 발생, `adp-watch` 가 `· ?` 로 렌더):
   ```json
   {"ts":"2026-06-12T13:09:05Z","type":"task_start","task_id":"T-1","agent":"backend-eng"}
   {"ts":"2026-06-12T13:13:43Z","type":"task_done","task_id":"T-1","status":"success","elapsed_sec":278}
   ```
   전체 타입 표는 `observability` skill §1 (spec §12.1). 첫 append 전에 `mkdir -p .artifacts/runs` (디렉토리 없으면 `>>` 가 실패 — pm 의 specs/, architect 의 plans/ 와 동일 패턴). 또한 각 task 의 출력 JSON 을 `.artifacts/runs/{slug}-impl-outputs/{task_id}.json` 으로 저장한다 — `/verify {slug}` 가 세션 경계를 넘어 Concordance Gate 입력으로 쓴다. **실행 시작 시(step 2 이전) 이 디렉토리를 비우고 시작**한다 — 개정 전 plan 의 잔존 출력이 게이트에 거짓 커버리지를 주장하는 것을 방지 (verify 쪽도 현재 plan 의 task_id 로 필터하지만, 생산자 측 위생이 1차 방어). 같은 이유로 **재실행 시에도 plan 의 전체 task 를 재실행**한다 — 부분 재개(이미 merge 된 task 건너뛰기)는 미지원이며, run-start 비우기와 게이트 완결성이 이 전제 위에 서 있다.
8. **실패 처리**: task 실패 시 실패 로그를 `${CLAUDE_PLUGIN_ROOT}/scripts/validate/classify-error-recovery.sh` 의 stdin 으로 넘겨 분류한다 — stdout 의 `{category, action}` 과 exit 코드(1 = 재시도, 2 = escalate; 항상 비-0 이므로 `|| rc=$?` 로 받기)를 따르고, **그 외 rc(126/127 등 스크립트 자체 실행 실패)는 `ENV_FAILURE` 로 사람에게** escalate 한다. 분류 표의 SSOT 는 `common-agent-rules` skill §8. 재시도 한도는 `config/pipeline.json` 의 `limits`.
