...
> **현재 상태: PR3 (MCP github 통합)**. 5개 PreToolUse 훅, Skills 시스템(`paths` 자동 활성화), project-ops 에이전트 (MCP github + gh CLI fallback). 자세한 설계는 [`multi-agent-pipeline-best-practices.md`](./multi-agent-pipeline-best-practices.md), 2026년 베스트 프랙티스 분석은 PR1 커밋 메시지 참조.
...
| GitHub 작업 (MCP github 서버 + `gh` CLI fallback — `scripts/gh/` 는 PR3에서 폐기) | working | §3.2 |
...
| **PR3 (이 커밋)** | MCP github 서버 통합 → `scripts/gh/*.sh` 폐기, `project-ops` 풀 에이전트 재정의 (MCP 우선 + gh CLI fallback), plugin.json mcpServers 선언 |
