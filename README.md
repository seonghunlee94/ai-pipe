...
> **현재 상태: PR2 (Skills 마이그레이션)**. 디렉토리 구조, 5개 PreToolUse 훅 (8 review rounds 정제), Skills 시스템(`paths` 자동 활성화) 사용. 자세한 설계는 [`multi-agent-pipeline-best-practices.md`](./multi-agent-pipeline-best-practices.md), 2026년 베스트 프랙티스 분석은 PR1 커밋 메시지 참조.
...
| `skills/{pm-rules,backend-conventions}/SKILL.md` (paths 비활성 — 본문 채워질 때 활성화) | stub | §6.1, §3-2 |
...
| **PR2 (이 커밋)** | `shared/agent-rules/`, `commands/` → `skills/{name}/SKILL.md` 마이그레이션, `paths` frontmatter 자동 활성화, agent의 SHARED_REF 블록 제거, plugin.json components 정리 |
